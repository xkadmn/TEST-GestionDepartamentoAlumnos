...
date_default_timezone_set('America/Buenos_Aires'); // Ajusta la zona horaria a tu ubicación
$hoy = date("Y-m-d");
?>
<?php ?>
...
<body>
  <script>
    ...
    $(document).ready(function() {
      ...
      $("#eliminar-pdf").click(function() { // CA01: *1 Una vez que se adjunta el archivo, no se puede eliminar //SE AGREGA BOTON DE ELIMINAR PDF E IMAGEN      
        $("#adjunto").val(""); // Elimina archivo PDF seleccionado
      });

      $("#eliminar-imagen").click(function() {
        $("#imagen").val(""); // Elimina imagen seleccionada
      });

      $('#fecha_publicacion').on('change', function() { // Actualizar el campo de fecha de vencimiento en base a la fecha de publicación
        ...
        if ($('#fecha_vencimiento').val() && $('#fecha_vencimiento').val() < fechaPublicacion) {
          $('#fecha_vencimiento').val(fechaPublicacion); // Ajusta la fecha de vencimiento si es anterior
        }
      });
    });
  </script>
      ...
      <div class="card-body">
        ...
        <form id="formulario">
          ...
          <div class="mb-3">
           ...
           <!-- CA01: *4 CREAR ANUNCIO con fecha anterior , SE PROCESA OK Y QUEDA ACTIVO EN LISTA DE AVISOS : se limita el calendario para seleccionar fecha a partir de hoy-->          
           <div class="mb-3">
            <label for="fecha_publicacion" class="form-label">Fecha de Publicacion:</label>
            <input type="date" class="form-control" name="fecha_publicacion" id="fecha_publicacion" aria-describedby="helpId" placeholder="Fecha de Publicacion" required min="<?php echo $hoy; ?>" value="<?php echo $hoy; ?>">            
          </div>
          <div class="mb-3">
            <label for="fecha_vencimiento" class="form-label">Fecha de Vencimiento:</label>
            <input type="date" class="form-control" name="fecha_vencimiento" id="fecha_vencimiento" aria-describedby="helpId" placeholder="Fecha de Vencimiento" required min="<?php echo $hoy; ?>" value="<?php echo $hoy; ?>">
          </div>
            <!-- CA01: *1 Una vez que se adjunta el archivo, no se puede eliminar SE AGREGA BOTON DE ELIMINAR PDF E IMAGEN -->        
          <div class="mb-3">
