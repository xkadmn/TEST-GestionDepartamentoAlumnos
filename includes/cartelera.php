...
//PRUEBA CA01 NRO 6,7,10 // Filtrar los datos para excluir los avisos inactivos, vencidos y con fecha de publicación futura
$datos_filtrados = array_filter($data, function ($item) use ($fecha_actual) {
    return ($item['fecha_vencimiento'] >= $fecha_actual) &&
           ($item["estado"] != "Inactivo") &&
           ($item['fecha_publicacion'] <= $fecha_actual); // Mostrar solo avisos con fecha de publicación hasta hoy
});


// Ordenar: fijados primero, luego por fecha de publicación en orden ascendente
usort($datos_filtrados, function ($a, $b) {
    if ($a['fijado'] === $b['fijado']) {
        return strcmp($a['fecha_publicacion'], $b['fecha_publicacion']);
    }
    return $a['fijado'] ? -1 : 1; // Los fijados se muestran primero
});
